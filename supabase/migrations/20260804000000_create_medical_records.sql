-- Migration: Create medical_records table with RLS and indexes
...
DROP POLICY IF EXISTS "Users can view medical records for their clinic" ON public.medical_records;
...
DROP POLICY IF EXISTS "Users can insert medical records for their clinic" ON public.medical_records;
...
DROP POLICY IF EXISTS "Users can update medical records for their clinic" ON public.medical_records;
CREATE POLICY "Users can update medical records for their clinic"
ON public.medical_records FOR UPDATE
USING (clinic_id = (auth.jwt() -> 'user_metadata' ->> 'clinic_id')::uuid);

DROP POLICY IF EXISTS "Users can delete medical records for their clinic" ON public.medical_records;
